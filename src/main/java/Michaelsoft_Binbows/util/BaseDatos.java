...
public class BaseDatos {
  ...
  public void eliminarUsuario(Usuario usuario) {
    ...
    boolean fueEliminado =
        usuarios.removeIf(
            u -> u.getCorreoElectronico().equalsIgnoreCase(usuario.getCorreoElectronico()));
    if (fueEliminado) {
      System.out.println("El usuario '" + usuario.getNombreUsuario() + "' ha sido eliminado.");
      guardarBaseDatos();
    } else {
      throw new IllegalArgumentException(
          "El usuario a eliminar no fue encontrado en la base de datos.");
    }
  }
  ...
  */
  ...
  public void actualizarUsuario(
      ...
      throws EdicionInvalidaException { // <--- CAMBIO 1: Acepta "nuevoCorreo"
    // --- VALIDACIONES ---
    ...
    if (usuarioExistePorNombre(nuevoNombre, correoOriginal)) {
      throw new EdicionInvalidaException(
          "El nombre '" + nuevoNombre + "' ya está en uso por otro usuario.", correoOriginal);
    }

    // 2. Validar que el nuevo correo no esté en uso por OTRO usuario. (¡ESTO ES NUEVO!)
    if (usuarioExistePorCorreo(
        nuevoCorreo, correoOriginal)) { // <-- CAMBIO 2: Usa el nuevo método de ayuda
      throw new EdicionInvalidaException(
          "El correo '" + nuevoCorreo + "' ya está registrado por otro usuario.", correoOriginal);
    }
    ...
    if (usuarioAActualizar == null) {
      throw new EdicionInvalidaException(
          "Error crítico: No se pudo encontrar al usuario para actualizar.", correoOriginal);
    }

    usuarioAActualizar.setNombreUsuario(nuevoNombre);
    usuarioAActualizar.setCorreoElectronico(
        nuevoCorreo); // <--- CAMBIO 3: Ahora actualiza el correo
  ...
  public void actualizarContraseñaUsuario(String correoUsuario, String nuevaContraseñaSinEncriptar)
    ...
    if (usuario == null) {
      ...
      throw new IllegalArgumentException(
          "No se encontró un usuario con el correo: " + correoUsuario);
    }
    ...
    guardarBaseDatos();
    System.out.println(
        "LOG: La contraseña del usuario '"
            + usuario.getNombreUsuario()
            + "' ha sido actualizada y encriptada.");
  }
