...
public class GestorLogros {
     ...
     * Este método se ejecuta una vez durante la inicialización de la clase.
     * Si se detectan IDs duplicados, lanza una excepción para prevenir inconsistencias.
     */
    private static void verificarUnicidadDeIds() {
        ...
        for (Logro logro : LOGROS_DISPONIBLES) {
            if (!idsVistos.add(logro.getId())) {
                throw new IllegalStateException("Error de configuración: ID de logro duplicado -> " + logro.getId());
            }
        }
        System.out.println("LOG: Verificación de logros completada. " + LOGROS_DISPONIBLES.size() + " logros únicos cargados.");
    }

    /**
     * Evalúa si un usuario cumple con las condiciones para desbloquear logros pendientes.
     * Compara el progreso del usuario contra todos los logros disponibles y retorna
     * aquellos que el usuario ha desbloqueado recientemente.
     *
    ...
    /**
     * Determina si un usuario específico cumple los requisitos para un logro dado.
     * Este método centraliza la lógica de evaluación de condiciones para mantener
     * el código modular y extensible.
     *
